...
# @Author : momo
# @File : daily_dialog_parser
# @Project : Social-Conversation-Synthesis
...
def fetch_only_dialog(json_list):
    ans = []
...
if __name__ == '__main__':
    # dd = DailyDialog()
    # ans = dd._generate_examples("./data/train.zip","train/dialogues_train.txt", "train/dialogues_act_train.txt", "train/dialogues_emotion_train.txt")
    # ans = list(ans)
    #
    # with open("./data/daily_dialog.json", "w", encoding="utf-8") as fp:
    #     json.dump(ans, fp, indent=4, ensure_ascii=False)

    with open("./data/daily_dialog.json", "r", encoding="utf-8") as fp:
        data = json.load(fp)

    ans = fetch_only_dialog(data)

    with open("data/daily_dialog/daily_dialog_only.json", "w", encoding="utf-8") as fp:
        json.dump(ans, fp, indent=4, ensure_ascii=False)
